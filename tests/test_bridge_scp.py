...
scp_cmd_module = importlib.import_module("inspire.cli.commands.bridge.scp_cmd")
...
def test_bridge_scp_download_success(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(
        cli_main, ["bridge", "scp", "--download", "/tmp/remote.txt", str(tmp_path / "local.txt")]
    )
...
def test_bridge_scp_recursive_flag(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli_main, ["bridge", "scp", "-r", str(src_dir), "/tmp/mydir"])
...
def test_bridge_scp_auto_recursive_for_directory(
    ...
    result = runner.invoke(cli_main, ["bridge", "scp", str(src_dir), "/tmp/autodir"])
...
def test_bridge_scp_local_path_not_found(tmp_path: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(cli_main, ["bridge", "scp", "/nonexistent/file.txt", "/tmp/test.txt"])
...
def test_bridge_scp_json_output(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli_main, ["--json", "bridge", "scp", str(local_file), "/tmp/test.txt"])
...
def test_bridge_scp_timeout(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(
        cli_main, ["bridge", "scp", str(local_file), "/tmp/test.txt", "--timeout", "5"]
    )
...
def test_bridge_scp_bridge_option(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(
        cli_main, ["bridge", "scp", str(local_file), "/tmp/test.txt", "--bridge", "gpu-main"]
    )
