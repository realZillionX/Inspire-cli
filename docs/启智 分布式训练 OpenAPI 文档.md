...
```bash
curl --location 'http://qz.sii.edu.cn/openapi/v1/train_job/create' \
...
- 请求方式：`POST`。
- 请求地址：`https://qz.sii.edu.cn/openapi/v1/train_job/create`。

#### 请求参数

| 参数 | 类型 | 是否必选 | 示例值 | 描述 |
| --- | --- | --- | --- | --- |
| name | String | 是 | test_api | 分布式训练任务名称。 |
...
| fault_tolerance_max_retry | Integer | 否 | 10 | 训练容错的最大重试次数（开启容错后必填）。 |
...
| image_type | String | 是 | SOURCE_OFFICIAL | 镜像类型，可选 SOURCE_PUBLIC、SOURCE_PRIVATE、SOURCE_OFFICIAL。 |
...
- 请求方式：`POST`。
- 请求地址：`https://qz.sii.edu.cn/openapi/v1/train_job/detail`。
...
```bash
curl --location --request POST 'https://qz.sii.edu.cn/openapi/v1/train_job/detail' \
  --header 'Authorization: Bearer xxxx' \
  --data-raw '{
    "job_id": "job-xxxx"
  }'
...
- 请求方式：`POST`。
- 请求地址：`https://qz.sii.edu.cn/openapi/v1/train_job/stop`。
...
```bash
curl --location --request POST 'https://qz.sii.edu.cn/openapi/v1/train_job/stop' \
  --header 'Authorization: Bearer xxxx' \
  --data-raw '{
    "job_id": "job-bf6c6a50-841b-4da5-aaa4-2f50af03xxxx"
  }'
```
