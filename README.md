...
> **注意：** `hpc create` 的 `--spec-id` 必须使用 HPC 的 `predef_quota_id`。优先运行 `inspire resources specs --usage hpc` 获取，或从已有 HPC 任务的 `inspire --json hpc status <job_id>` → `slurm_cluster_spec.predef_quota_id` 提取。默认的 `resources specs` 输出是 notebook／DSW 规格；创建 HPC 任务时不要混用。`--image` 必须用完整 docker 地址。
...
- `inspire resources specs` 默认查询 notebook／DSW 规格；只有显式加 `--usage hpc` 才会返回 HPC 的 `predef_node_specs`。
