...
| `inspire bridge exec "<cmd>"`    | 在远端 `INSPIRE_TARGET_DIR` 下执行命令，支持 `--stdin -- bash -s` |
...
- `bridge exec` 支持 `--stdin` / `--bash-stdin`，也会在检测到管道或重定向输入时自动透传 stdin 到远端命令；这适合 `bash -s` 之类需要读取 stdin 的场景。
- `image set-default` 现在会向上查找现有项目的 `.inspire/config.toml`，因此在子目录中执行也会更新项目根配置，而不是误写到嵌套目录。
